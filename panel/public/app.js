    ...
    : `<div style="max-height:420px;overflow-y:auto"><table class="grid"><thead><tr><th></th><th>模组</th><th>说明</th><th>类型</th><th>来源（本机地址）</th><th>状态</th><th>操作</th></tr></thead><tbody>` +
      mods.map((m) => `<tr>
        ...
        <td style="min-width:180px"><div style="display:flex;align-items:center;gap:8px">
          ${m.hasIcon ? `<img src="local-icon?id=${esc(m.id)}" style="width:36px;height:36px;object-fit:contain;border-radius:6px;border:1px solid var(--border);flex-shrink:0" onerror="this.style.display='none'">` : m.preview ? `<img src="${proxyImg(m.preview)}" title="官网预览图（本地无图标）" style="width:36px;height:36px;object-fit:cover;border-radius:6px;border:1px solid var(--border);flex-shrink:0" onerror="this.style.display='none'">` : ""}
